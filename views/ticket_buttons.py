...
class TicketButtonsView(View):
        ...
        if log_cfg:
            ...
            if log_ch:
                if len(transcript) <= 1900:
                    ...
                    await log_ch.send(
                        f"Transcript for ticket {t_id} is too long for one message.\n"
                        f"```\n{transcript[:1900]}\n```"
                    )
    ...
    async def transcript(self, interaction: discord.Interaction, button: discord.Button):
        row = get_ticket_by_channel(interaction.guild.id, interaction.channel.id)
        if not row:
            await interaction.response.send_message("Not a ticket channel.", ephemeral=True)
            return
        ...
        if log_cfg:
            ...
            if log_ch:
                if len(transcript) <= 1900:
                    await log_ch.send(f"Transcript for ticket {row[0]}:\n```\n{transcript}\n```")
                else:
                    await log_ch.send(
                        f"Transcript for ticket {row[0]} is too long for one message.\n"
                        f"```\n{transcript[:1900]}\n```")

        await interaction.response.send_message("Transcript sent.", ephemeral=True)

    @discord.ui.button(label="Delete", style=discord.ButtonStyle.grey, custom_id="ticket_delete")
