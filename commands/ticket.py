...
from db import (
    ...
    close_ticket, delete_ticket, set_claim, reopen_ticket, get_ticket, get_ticket_by_channel,
)
...
async def setup(bot: commands.Bot):
    ...
    # ---- setup ----
    @ticket_group.command(name="setup", description="Create/configure the ticket panel")
    async def setup_cmd(interaction: Interaction):
        view = TicketPanelView()
        embed = Embed(title="Ticket System", description="Select a category to open a ticket.", color=discord.Color.green())
        ...
        if cfg is None:
            ...
            set_guild_config(interaction.guild.id, ticket_panel_message_id=msg.id)
    ...
    async def close_cmd(interaction: Interaction):
        row = get_ticket_by_channel(interaction.guild.id, interaction.channel.id)
        if not row:
        ...
        if staff_role and staff_role not in interaction.user.roles and not is_creator:
            await interaction.response.send_message("You cannot close this ticket.", ephemeral=True)
            return
        close_ticket(t_id)
        embed = Embed(description=f"Ticket closed by {interaction.user.mention}", color=discord.Color.red())
        ...
        if log_cfg:
            ...
            if log_ch:
                await log_ch.send(f"Transcript for ticket {t_id}:
```{transcript}```")
        # rename channel
    ...
    # ---- delete ----
    @ticket_group.command(name="delete", description="Permanently delete the ticket")
    ...
    # ---- claim ----
    @ticket_group.command(name="claim", description="Claim the ticket")
    ...
    # ---- unclaim ----
    @ticket_group.command(name="unclaim", description="Remove the claim")
    async def unclaim_cmd(interaction: Interaction):
        ...
        c = conn.cursor()
        c.execute("UPDATE tickets SET claimed_by=NULL WHERE ticket_id=?", (row[0],))
        conn.commit()
        conn.close()
        await interaction.response.send_message("Claim removed.", ephemeral=True)

    # ---- add ----
    @ticket_group.command(name="add", description="Add a user to the ticket")
    async def add_cmd(interaction: Interaction, user: Member):
        row = get_ticket_by_channel(interaction.guild.id, interaction.channel.id)
        if not row:
            await interaction.response.send_message("Not a ticket channel.", ephemeral=True)
            return
    ...
    async def remove_cmd(interaction: Interaction, user: Member):
        await interaction.channel.set_permissions(user, read_messages=False, send_messages=False)
        await interaction.response.send_message(f"{user.mention} removed from ticket.", ephemeral=True)

    # ---- rename ----
    @ticket_group.command(name="rename", description="Rename the ticket channel")
    async def rename_cmd(interaction: Interaction, name: str):
        await interaction.channel.edit(name=name)
        await interaction.response.send_message(f"Channel renamed to {name}.", ephemeral=True)

    # ---- transcript ----
    @ticket_group.command(name="transcript", description="Generate a transcript")
    async def transcript_cmd(interaction: Interaction):
        ...
        if log_cfg:
            log_ch = interaction.guild.get_channel(log_cfg)
            if log_ch:
                await log_ch.send(f"Transcript for ticket {interaction.channel.id}:
```{transcript}```")
        await interaction.response.send_message("Transcript sent to log channel.", ephemeral=True)
    ...
    # ---- stats ----
    @ticket_group.command(name="stats", description="Show ticket statistics")
    async def stats_cmd(interaction: Interaction):
        conn = sqlite3.connect("tickets.db")
        c = conn.cursor()
        c.execute("SELECT COUNT(*) FROM tickets WHERE guild_id=? AND closed_at IS NULL", (interaction.guild.id,))
        open_tix = c.fetchone()[0]
        c.execute("SELECT COUNT(*) FROM tickets WHERE guild_id=? AND closed_at IS NOT NULL", (interaction.guild.id,))
        closed_tix = c.fetchone()[0]
        conn.close()
        embed = Embed(title="Ticket Stats", description=f"Open: {open_tix}\nClosed: {closed_tix}", color=discord.Color.blue())
        await interaction.response.send_message(embed=embed, ephemeral=True)

    # ---- config ----
    @ticket_group.command(name="config", description="Configure ticket settings")
    async def config_cmd(interaction: Interaction, *, setting: str = None):
        if not setting:
            await interaction.response.send_message("Usage: /ticket config staff_role <role>", ephemeral=True)
            return
        parts = setting.split()
        if parts[0] == "staff_role" and len(parts) >= 2:
            ...
            if not role:
                ...
                return
            set_guild_config(interaction.guild.id, staff_role_id=role.id)
